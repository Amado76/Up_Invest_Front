...
void main() {
  ...
  final IUserAssetRepository userAssetRepository =
      UserAssetRepository(httpClientAdapter: dioAdapterMock);
  ...
  group('[deleteAllUserAsset]', () {
    test('Should call httpClientAdapter.delete', () async {
      //arrange
      when(() => dioAdapterMock.delete(
            path: UrlsPaths.userAssetsUrlPath,
            headers: {
              'userid': 1,
              'authtoken': 'authToken',
            },
  ...
  group('[getUserAssets]', () {
    test('should return a list of Userassets', () async {
      ...
      //Act
      List<UserAssetModel> result = await userAssetRepository.getUserAssets(
          userId: 1, authToken: 'authToken');
      //Assert
      expect(result, isA<List<UserAssetModel>>());
